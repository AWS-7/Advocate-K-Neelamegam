...
async function main() {
  ...
  const desktopEntries = await optimizeSlides(desktopHeroSources, DESKTOP_MAX_WIDTH);
...
export const heroCarouselDesktopSlides = ${JSON.stringify(
    desktopEntries.map(({ bytes: _bytes, ...rest }) => rest),
    null,
    2,
...
`;
  ...
  const totalKb = desktopEntries.reduce((sum, entry) => sum + entry.bytes, 0) / 1024;
  console.log(`\n${desktopEntries.length} desktop hero images optimized — total ${totalKb.toFixed(1)} KB`);
}
